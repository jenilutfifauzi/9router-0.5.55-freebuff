...
async function requestSession(token, model, proxyOptions) {
  ...
  if (status === "active") {
    // Limited Freebuff tier may return an active fallback session (currently
    // MiMo) even when x-freebuff-model requested another model. Never cache
    // that session under the requested model; it causes misleading cross-model
    // routing and later 409s.
    if (data.model && data.model !== model) {
      const err = new Error(`Freebuff session model mismatch: ${JSON.stringify({ status: "model_locked", currentModel: data.model, requestedModel: model })}`);
      err.status = 409;
      throw err;
    }
